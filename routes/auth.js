...
// ========================
// REGISTER USER (UPDATED with custodial wallet)
// ========================
router.post('/register', async (req, res) => {
  try {
    ...
    // Generate REAL wallet for new user
    const wallet = walletService.generateUserWallet();
    ...
    // Return user data with deposit address and internal balance
    res.status(201).json({
      success: true,
...
// ========================
// LOGIN USER (UPDATED with custodial wallet)
// ========================
...
// ========================
// REFRESH TOKEN (UPDATED with custodial wallet)
// ========================
...
// ========================
// GET CURRENT USER (UPDATED with custodial wallet)
// ========================
...
router.get('/admin/nfts', adminAuth, async (req, res) => {
  try {
    ...
    const nfts = await NFT.find({})
      .populate('owner', 'email fullName depositAddress')
      .sort({ createdAt: -1 });
...
// ========================
// ADMIN SEARCH USERS BY WALLET ADDRESS (UPDATED)
// ========================
...
// Update user internal balance (replaces old balance endpoints)
router.put('/admin/users/:userId/balance', adminAuth, async (req, res) => {
  try {
    const { userId } = req.params;
    const { internalBalance } = req.body;
    
    console.log(`🔍 Admin updating user ${userId} internal balance to ${internalBalance}`);
    
    if (internalBalance === undefined || internalBalance < 0) {
      return res.status(400).json({ error: 'Valid internal balance required' });
    }
    ...
    res.json({
      success: true,
      message: 'Internal balance updated successfully',
      user: {
        _id: user._id,
        email: user.email,
        depositAddress: user.depositAddress,
...
router.get('/admin/tickets', adminAuth, async (req, res) => {
  try {
    ...
    const tickets = await Ticket.find({})
      .populate('user', 'email fullName depositAddress')
      .sort({ createdAt: -1 });
    ...
    res.json({
      ...
      tickets: tickets.map(ticket => ({
        ...
        userWallet: ticket.user?.depositAddress || 'Unknown',
